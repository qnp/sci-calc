...
function init() {
	rootPropCopy = Object.getOwnPropertyNames(global);
...
function customEval(cmd, ctx, filename, callback) {
	if (helpInterface) {
		...
		var rootAttack = false;
		rootPropCopy.forEach(function(prop) { // check if cmd involves in any part a protected root name
			if (cmd.match('^'+prop+'(\\.|$)')) rootAttack = true;
		});
		if (rootAttack) {
			console.log('RootError: Cannot access global variable'.red+listProtected);
			...
			if (!cmd) {
				...
				try {
					if (goodPowa) {
						var result = root.eval(cmd);
						if (typeof(result) === 'undefined') {
							repl.repl.displayPrompt();
						} else if (typeof(result) === 'function') {
							logFunction(result);
						} else if (typeof(result) === 'number') {
							root.Ans = root.ans = result;
...
function notInRoot(name) {
	...
	rootPropCopy.forEach(function(prop) {
		if (name === prop) {
			not = false;
			console.log('RootError: Cannot access root variable'.red+listProtected);
		}
	});
	return not;
}
